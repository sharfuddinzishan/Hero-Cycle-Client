...
const MyOrders = () => {
    ...
    useEffect(() => {
        setLoader(true);
        let url = `https://hero-cycle-server-production.up.railway.app/user/orders?email=${user?.email}`
        axios.get(url)
            .then(result => {
    ...
    const handleCancel = (orderID, orderStatus) => {
        ...
        if (prompt === true) {
            setRefreshed(false);
                ...
                axios.put(`https://hero-cycle-server-production.up.railway.app/order/${orderID}?action=cancel`, { headers })
                    .then(result => {
                        // console.log(result)
    ...
    const handleDelete = (orderID, orderStatus) => {
        ...
        if (prompt === true) {
            setRefreshed(false);
            // console.log(headers)
            orderStatus !== 'confirm' && axios.delete(`https://hero-cycle-server-production.up.railway.app/order/${orderID}`, { headers })
                .then(result => {
                    // console.log(result)
