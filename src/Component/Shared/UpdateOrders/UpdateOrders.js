...
const UpdateOrders = (props) => {
    ...
    useEffect(() => {
        ...
        setSuccess(false)
        setError(false)
        axios.get(`https://hero-cycle-server-production.up.railway.app/orders/${getOrderID}`)
            .then(result => {
    ...
    const { isAdmin, loadingAdmin } = useFirebase()
    ...
    const handleSubmit = (e) => {
        ...
        setError(false)
        setRefreshed(false)
        axios.put('https://hero-cycle-server-production.up.railway.app/order', getSingleOrderDetails, { headers })
            .then(result => {
                // console.log(result)
